    ...
    override suspend fun translate(
        ...
        client.newCall(request).execute().use { response ->
            ...
            if (!response.isSuccessful) {
                throw IOException("OpenAI HTTP ${response.code}: ${parseError(bodyString)}")
            }

            if (bodyString.isBlank()) {
                throw IOException("OpenAI: пустой ответ")
            }

            val json = JSONObject(bodyString)
            val choices = json.optJSONArray("choices")
                ?: throw IOException("OpenAI: нет поля choices")
            if (choices.length() == 0) {
                throw IOException("OpenAI: choices пустой")
            }
            ...
            if (content.isNullOrEmpty()) {
                throw IOException("OpenAI: пустой content")
            }
